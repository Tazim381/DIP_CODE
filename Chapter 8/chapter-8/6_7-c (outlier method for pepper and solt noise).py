...
def apply_outlier_method(image):
    ...
    for i in range(height):
        for j in range(width):
            ...
            outlier_value = sorted_values[len(sorted_values) // 2]  # Use the median value as the outlier value
            filtered_image[i, j] = outlier_value
...
# Apply the outlier method to denoise the images
denoised_image_5 = apply_outlier_method(noisy_image_5)
denoised_image_10 = apply_outlier_method(noisy_image_10)
denoised_image_20 = apply_outlier_method(noisy_image_20)
